...
// Global variables
int i, thread_count = 8, sample_size = 1000, list_size, suppress_output;
		...
		void *Thread_work(long rank) {
			...
			for (i = offset; i < (offset + local_sample_size); i++) {
				do {
					...
					seed = (my_rank * local_chunk_size) + (rand() % local_chunk_size);
			...
			local_data = (int *) malloc(local_chunk_size * sizeof(int));
